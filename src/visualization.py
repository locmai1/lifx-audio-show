...
def visualize_sine(y):
    """Effect that expands from the center with increasing sound energy"""
    global p
    global shift
    global prev_fps_update
    y = np.copy(y)
    gain.update(y)
    y /= gain.value
    ...
    if time.time() - 0.05 > prev_fps_update:
        prev_fps_update = time.time()
        shift += 0.1
    sinwave = 100*np.sin(0.103*length+shift)+100
...
def microphone_update(audio_samples): 
    ...
    if config.DISPLAY_FPS:
        ...
        if time.time() - 2 > prev_fps_update:
            prev_fps_update = time.time()
...
if __name__ == '__main__':
    if config.USE_GUI:
        ...
        # Mel filterbank plot
        fft_plot = layout.addPlot(title='Filterbank Output', colspan=5)
        ...
        # Visualization plot
        layout.nextRow()
        led_plot = layout.addPlot(title='Visualization Output', colspan=5)
        ...
        def freq_slider_change(tick):
            ...
            dsp.create_mel_bank()
        freq_slider = pg.TickSliderItem(orientation='bottom', allowAdd=False, allowRemove=False)
        ...
        def bright_slider_change(tick):
            ...
            config.MAX_BRIGHTNESS = int(config.BRIGHTNESS * 65535)
            bright_label.setText('Max brightness: {:.0f}% ({:d})'.format(
                newBrightness*100,
                config.MAX_BRIGHTNESS))
        bright_slider = pg.TickSliderItem(orientation='bottom', allowAdd=False, allowRemove=False)
        bright_slider.tickMoveFinished = bright_slider_change
        bright_slider.addTick(config.BRIGHTNESS)
        bright_label.setText('Max brightness: {:.0f}% ({:d})'.format(
            bright_slider.tickValue(0)*100,
            config.MAX_BRIGHTNESS))

        # Saturation label
        sat_label = pg.LabelItem('')
        # Saturation slider
        def sat_slider_change(tick):
            newSaturation = sat_slider.tickValue(0)
            config.SATURATION = newSaturation
            config.MAX_SATURATION = int(config.SATURATION * 65535)
            sat_label.setText('Max saturation: {:.0f}% ({:d})'.format(
                newSaturation*100,
                config.MAX_SATURATION))
        sat_slider = pg.TickSliderItem(orientation='bottom', allowAdd=False, allowRemove=False)
        sat_slider.tickMoveFinished = sat_slider_change
        sat_slider.addTick(config.SATURATION)
        sat_label.setText('Max saturation: {:.0f}% ({:d})'.format(
            sat_slider.tickValue(0)*100,
            config.MAX_SATURATION))
        ...
        def energy_click(x):
            ...
            sine_label.setText('Sine', color=inactive_color)
        ...
        def sine_click(x):
            config.FLAT_MODE = False
            global visualization_effect
            visualization_effect = visualize_sine
            energy_label.setText('Energy', color=inactive_color)
            scroll_label.setText('Scroll', color=inactive_color)
            spectrum_label.setText('Spectrum', color=inactive_color)
            sine_label.setText('Sine', color=active_color)
            flat_label.setText('Flat', color=inactive_color)
            off_label.setText('Off', color=inactive_color)
            on_label.setText('On', color=inactive_color)
            kill_label.setText('Kill', color=inactive_color)
        ...
        sine_label.mousePressEvent = sine_click
        ...
        # Layout
        layout.nextRow()
        layout.addItem(freq_label, colspan=5)
        layout.nextRow()
        layout.addItem(freq_slider, colspan=5)

        layout.nextRow()
        layout.addItem(bright_label, colspan=5)
        layout.nextRow() 
        layout.addItem(bright_slider, colspan=5)

        layout.nextRow()
        layout.addItem(sat_label, colspan=5)
        layout.nextRow() 
        layout.addItem(sat_slider, colspan=5)

        layout.nextRow()
        layout.addItem(pattern_label, colspan=5)
        layout.nextRow()
        layout.addItem(energy_label)
        layout.addItem(scroll_label)
        layout.addItem(sine_label)
